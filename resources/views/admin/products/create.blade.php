...
@section('content')
    <div class="card shadow-sm">
        <div class="card-body">
            <h1 class="h4 mb-3">Thêm mới sản phẩm</h1>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="cateid" class="form-select">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('cateid') == $category->id ? 'selected' : '' }}>{{ $category->catename }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="brandid" class="form-select">
                                ...
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->id }}" {{ old('brandid') == $brand->id ? 'selected' : '' }}>{{ $brand->brandname }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
